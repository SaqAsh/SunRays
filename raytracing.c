...
void fill_rays(SDL_Surface *surface, Ray rays[RAYS_NUMBER], uint32_t color,
               Circle obstacle) {
  ...
  for (int i = 0; i < RAYS_NUMBER; i++) {
    ...
    while (!end_of_screen && !object_hit) {
      ...
      double distance_squared =
          pow(x_draw - obstacle.x, 2) + pow(y_draw - obstacle.y, 2);
...
int main() {
  ...
  Circle shadow = (Circle){500, 300, 140};
  ...
  while (running) {
    ...
    fill_rays(surface, rays, RAY_COLOR, shadow);
